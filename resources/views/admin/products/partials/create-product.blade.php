    ...
    x-data="{
        ...
        normalizeVariants(data){
            ...
            if(Array.isArray(data)){
                data.forEach(v=>{
                    if(!v || v.size_id===undefined || v.size_id===null){
                        return;
                    }
        ...
        normalizeSizeIds(data,variants={}){
            ...
            if(Array.isArray(data)){
                ...
                ids=Object.values(data).map(id=>String(id));
            }

            if(ids.length===0){
                ids=Object.keys(variants).map(id=>String(id));
            }
        ...
        syncVariants(){
            @foreach($sizes as $size)
                if(!this.form.variants['{{ $size->id }}']){
                    this.form.variants['{{ $size->id }}']={price:'',stock:''};
                }else{
                    this.form.variants['{{ $size->id }}'].price ??='';
                    this.form.variants['{{ $size->id }}'].stock ??='';
                }
            @endforeach
        },
        ...
        validateForm(){
            ...
            this.form.size_ids.forEach(id=>{
                ...
                if(!variant){
                    this.errors['variants.'+id]='Data ukuran belum lengkap.';
                    return;
                }

                if(variant.price===''||variant.price===null||variant.price===undefined){
                    this.errors['variants.'+id+'.price']='Harga ukuran wajib diisi.';
                }

                if(variant.stock===''||variant.stock===null||variant.stock===undefined){
                    this.errors['variants.'+id+'.stock']='Stok ukuran wajib diisi.';
                }
            });
        ...
        async submitForm(e){
            ...
            try{
                ...
                if(response.status===422){
                    ...
                    throw new Error(
                        data.message||'Mohon periksa kembali data produk.'
                    );
                }

                if(!response.ok){
                    throw new Error(
                        data.message||'Gagal menyimpan data produk.'
                    );
                }
        ...
        openEdit(product){
            ...
            let sizeIds=this.normalizeSizeIds(
                product?.size_ids||[],
                variants
            );
...
<div 
    ...
    x-transition:leave="transition transform duration-500 ease-[cubic-bezier(0.4,0,0.2,1)]" 
    x-transition:leave-start="translate-x-0" 
    x-transition:leave-end="translate-x-full" 
    class="fixed right-0 top-0 z-[100] flex h-screen w-full flex-col bg-white shadow-2xl sm:max-w-[520px]" 
    style="display: none;"
>
    {{-- HEADER --}}
        ...
        <div class="flex-1 space-y-4 overflow-y-auto bg-slate-100 p-4 sm:space-y-6 sm:p-6">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                ...
                <div>
                    ...
                    <template x-if="errors.name">
                        <p class="mt-1.5 text-xs text-red-500" x-text="errors.name"></p>
                    </template>
                    @error('name')
                ...
                <div class="mt-4 grid grid-cols-1 gap-4 sm:mt-6 sm:grid-cols-2">
                    <div>
                        ...
                        <template x-if="errors.category_id">
                            <p class="mt-1.5 text-xs text-red-500" x-text="errors.category_id"></p>
                        </template>
                        @error('category_id')
                            <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <template x-if="errors.product_code">
                            <p class="mt-1.5 text-xs text-red-500" x-text="errors.product_code"></p>
                        </template>
                        @error('product_code')
                            <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                        @enderror
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                    ...
                    @foreach($sizes as $size)
                        <label class="cursor-pointer">
                            ...
                            <div class="flex h-[46px] items-center justify-between rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-700 transition-all duration-200 hover:border-[#AE7C18]/50 peer-checked:border-[#AE7C18] peer-checked:bg-[#AE7C18]/10 peer-checked:text-[#AE7C18]">
                                <span>{{ $size->name }}</span>
                                <span 
                                    x-show="form.size_ids.includes('{{ $size->id }}') || form.size_ids.includes({{ $size->id }})" 
                                    x-cloak 
                ...
                <template x-if="errors.size_ids">
                    <p class="mt-2 text-xs font-medium text-red-500" x-text="errors.size_ids"></p>
                </template>

                <p class="mt-2 text-xs text-slate-400">Pilih minimal satu ukuran yang tersedia untuk produk ini.</p>

                <div x-show="form.size_ids.length > 0" x-cloak class="mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
                    <span class="text-xs font-medium text-slate-500">Ukuran dipilih</span>
                    <span class="rounded-lg bg-[#AE7C18]/10 px-2.5 py-1 text-xs font-semibold text-[#AE7C18]" x-text="form.size_ids.length + ' ukuran'"></span>
                </div>
            </div>
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input 
                                            ...
                                            :class="errors['variants.{{ $size->id }}.price'] ? 'border-red-400 focus:border-red-500 focus:ring-red-500/10' : ''"
                                        >
                                    </div>
                                    <template x-if="errors['variants.{{ $size->id }}.price']">
                                        <p class="mt-1.5 text-xs text-red-500" x-text="errors['variants.{{ $size->id }}.price']"></p>
                                    </template>
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        :class="errors['variants.{{ $size->id }}.stock'] ? 'border-red-400 focus:border-red-500 focus:ring-red-500/10' : ''"
                                    >
                                    <template x-if="errors['variants.{{ $size->id }}.stock']">
                                        <p class="mt-1.5 text-xs text-red-500" x-text="errors['variants.{{ $size->id }}.stock']"></p>
                                    </template>
                    ...
                    <div class="flex gap-3">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs leading-5 text-blue-600">
                                Setiap ukuran memiliki harga dan stok masing-masing. Perubahan harga atau stok tidak memengaruhi ukuran lainnya.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                ...
                <div class="grid grid-cols-2 gap-2 rounded-xl bg-slate-100 p-1">
                    <button 
                        ...
                        :class="form.status === 'Aktif' ? 'bg-white text-[#AE7C18] shadow-sm' : 'text-slate-500 hover:text-slate-700'" 
                        class="inline-flex h-[46px] items-center justify-center rounded-lg text-sm font-semibold transition-all duration-200"
                    >
                        <span class="mr-2 h-2 w-2 rounded-full" :class="form.status === 'Aktif' ? 'bg-emerald-500' : 'bg-slate-300'"></span>
                        Aktif
                    </button>
                    ...
                    <button 
                        ...
                        :class="form.status === 'Tidak Aktif' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'" 
                        class="inline-flex h-[46px] items-center justify-center rounded-lg text-sm font-semibold transition-all duration-200"
                    >
                        <span class="mr-2 h-2 w-2 rounded-full" :class="form.status === 'Tidak Aktif' ? 'bg-red-500' : 'bg-slate-300'"></span>
                        Tidak Aktif
                    </button>
                </div>

    ...
    <div x-show="open" x-transition:enter="transition transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition transform duration-300 ease-in-out" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full" class="fixed right-0 top-0 z-[100] flex h-screen w-full sm:max-w-[520px] flex-col bg-white shadow-2xl" style="display:none;">
        ...
        <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4 sm:px-7 sm:py-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="mt-0.5 text-xs text-slate-500 sm:mt-1 sm:text-sm">Organize your product categories.</p>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
                <div class="mb-4 flex items-start gap-3 sm:mb-6 sm:gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-0.5 text-xs text-slate-500 sm:mt-1 sm:text-sm">Basic information about this category.</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <x-admin.input x-model="form.name" placeholder="e.g Football Jersey" />
                </div>

                {{-- Slug --}}
                <div class="mt-4 sm:mt-6">
                    <label class="mb-2 block text-sm font-medium text-slate-700">Slug</label>
                    <x-admin.input x-model="form.slug" placeholder="football-jersey" />
                </div>

                {{-- Description --}}
                <div class="mt-4 sm:mt-6">
                    <label class="mb-2 block text-sm font-medium text-slate-700">Description</label>
                    <x-admin.textarea rows="4" x-model="form.description" placeholder="Category description..." />
                </div>

                {{-- Status --}}
                <div class="mt-4 sm:mt-6">
                    <label class="mb-2.5 block text-sm font-medium text-slate-700 sm:mb-3">Status</label>
                    <div class="flex gap-2.5 sm:gap-3">
                        <button @click="form.status='Active'" type="button" class="flex-1 rounded-xl border px-3 py-2.5 text-sm font-medium transition-all duration-200 sm:px-4 sm:py-3 sm:text-base" :class="form.status==='Active' ? 'border-emerald-600 bg-emerald-600 text-white shadow-lg shadow-emerald-500/20' : 'border-slate-300 bg-white text-slate-700 hover:border-emerald-300 hover:bg-emerald-50'">
                            Active
                        </button>
                        <button @click="form.status='Inactive'" type="button" class="flex-1 rounded-xl border px-3 py-2.5 text-sm font-medium transition-all duration-200 sm:px-4 sm:py-3 sm:text-base" :class="form.status==='Inactive' ? 'border-red-500 bg-red-500 text-white shadow-lg shadow-red-500/20' : 'border-slate-300 bg-white text-slate-700 hover:border-red-300 hover:bg-red-50'">
                            Inactive
                        </button>
                    </div>
                </div>
        ...
        <div class="flex items-center justify-between border-t border-slate-200 bg-white px-5 py-4 sm:px-6 sm:py-5">
            ...
            <button @click="
                ...
                setTimeout(()=>{
                    $dispatch('toast',{
                        ...
                        message: mode==='create' ? 'Category created successfully.' : 'Category updated successfully.'
                    });
                },500);
            " class="rounded-xl bg-[#AE7C18] px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-[#AE7C18]/20 transition-all duration-300 hover:bg-[#96690F] hover:shadow-xl sm:px-6 sm:py-3 sm:text-base">
                <span x-text="mode==='create' ? 'Save Category' : 'Update Category'"></span>
            </button>
        </div>

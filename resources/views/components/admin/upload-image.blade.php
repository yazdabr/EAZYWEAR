...
<div x-data="{
            ...
            this.items=images.slice(0,this.maxFiles).map((image,index)=>{
                if(typeof image==='object'){
                    return {
                        ...
                        name:image.name??('Gambar '+(index+1)),
                        existing:true
                    };
                }
        ...
        if(files.length>remaining){
            ...
            window.dispatchEvent(new CustomEvent('toast',{
                detail:{
                    type:'error',
                    title:'Upload Gagal',
                    message:`Maksimal ${this.maxFiles} foto produk.`
                }
            }));
        ...
        if(invalidFile){
            ...
            window.dispatchEvent(new CustomEvent('toast',{
                detail:{
                    type:'error',
                    title:'Upload Gagal',
                    message:'Ukuran setiap foto maksimal 10 MB.'
                }
            }));
    ...
    removeImage(index){
        ...
        if(item&&!item.existing&&item.file){
            const fileIndex=this.newFiles.indexOf(item.file);
    ...
    existingIds(){
        return this.items
            .filter(item=>item.existing&&item.id)
            .map(item=>item.id);
    }
}" class="w-full">
    ...
    <div
        ...
        class="relative flex min-h-[140px] sm:min-h-[220px] cursor-pointer flex-col items-center justify-center rounded-xl sm:rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 p-4 sm:p-8 text-center transition hover:border-[#AE7C18] hover:bg-[#AE7C18]/5"
    ...
    >
        <div class="grid grid-cols-3 gap-2 sm:gap-3">
            ...
            <template
                ...
                :key="item.existing ? 'existing-'+item.id : 'new-'+index"
            >
                <div class="group relative overflow-hidden rounded-lg sm:rounded-xl border border-slate-200 bg-slate-100">
                    ...
                    <div class="absolute inset-x-0 bottom-0 bg-black/60 px-1.5 py-1 sm:px-3 sm:py-2">
                        <p
                            class="truncate text-[10px] sm:text-xs font-medium text-white"
                            x-text="index===0?'Foto Utama':'Foto '+(index+1)"
                        ></p>
                    </div>
                    ...
                    <button
                        ...
                        class="absolute right-1 top-1 sm:right-2 sm:top-2 flex h-6 w-6 sm:h-8 sm:w-8 items-center justify-center rounded-md sm:rounded-lg bg-black/60 text-white transition hover:bg-red-500"
                    >
            ...
            <button
                x-show="items.length<maxFiles"
                type="button"
                @click="openFilePicker()"
                class="flex aspect-square flex-col items-center justify-center rounded-lg sm:rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 text-center transition hover:border-[#AE7C18] hover:bg-[#AE7C18]/5"
            >
                ...
                <span class="mt-1 sm:mt-2 text-[10px] sm:text-xs font-semibold text-slate-600">
                    Tambah
                </span>

                <span
                    class="mt-0.5 text-[9px] sm:text-[10px] text-slate-400"
                    x-text="(maxFiles-items.length)+' slot'"
                ></span>
            </button>
